...
EXPORT m64p_error CALL CoreStartup(int APIVersion, const char *ConfigPath, void (*PumpEvents)(), void *Context,
...
EXPORT m64p_error CALL CoreDoCommand(m64p_command Command, int ParamInt, void *ParamPtr)
{
    ...
    switch(Command)
    {
        ...
        case M64CMD_ROM_OPEN:
            ...
            rval = open_rom((unsigned char *) ParamPtr, ParamInt);
            if (rval == M64ERR_SUCCESS)
            {
                l_ROMOpen = 1;
                cheat_init();
        ...
        case M64CMD_SET_SPEED:
            return main_set_speed(ParamInt);
...
EXPORT void CALL CoreSetGSButton(int pressed)
{
    KbdGamesharkPressed = pressed;
}
...
EXPORT m64p_error CALL CoreDeleteCheat(const char *CheatName)
{
    if (!l_CoreInit)
        return M64ERR_NOT_INIT;
    if (CheatName == NULL)
    ...
    if (cheat_set_enabled(CheatName, 0))
        return M64ERR_SUCCESS;
