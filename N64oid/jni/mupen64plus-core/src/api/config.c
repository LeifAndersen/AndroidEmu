...
static char    *l_ConfigRootDir = NULL;
...
m64p_error ConfigInit(const char *ConfigRootDir)
{
    ...
    l_ConfigRootDir = strdup(ConfigRootDir);
    if (l_ConfigRootDir == NULL)
...
m64p_error ConfigShutdown(void)
{
    ...
    if (l_ConfigRootDir != NULL)
    {
        free(l_ConfigRootDir);
        l_ConfigRootDir = NULL;
    }
...
EXPORT const char * CALL ConfigGetSharedDataFilepath(const char *filename)
{
  ...
  snprintf(path, sizeof(path), "%s/share/%s", l_ConfigRootDir, filename);
  return path;
}
...
{
  static char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/config/", l_ConfigRootDir);
  mkdir(path, 0700);
  return path;
}
...
{
  static char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/", l_ConfigRootDir);
  return path;
}
...
{
  static char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/cache/", l_ConfigRootDir);
  mkdir(path, 0700);
  return path;
}
