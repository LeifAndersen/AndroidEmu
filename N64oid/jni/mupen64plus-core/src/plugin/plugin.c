...
void (*playAudio)() = dummyaudio_PlayAudio;
...
static m64p_error plugin_connect_input(m64p_dynlib_handle plugin_handle)
{
    ...
    else
    {
        ...
        if (getVersion == NULL || controllerCommand == NULL || getKeys == NULL || initiateControllers == NULL ||
            readController == NULL || romOpen_input == NULL || romClosed_input == NULL)
        {
...
static m64p_error plugin_connect_audio(m64p_dynlib_handle plugin_handle)
{
    ...
    if (plugin_handle == NULL)
    {
        ...
        playAudio = dummyaudio_PlayAudio;
    ...
    else
    {
        ...
        playAudio = (ptr_PlayAudio) osal_dynlib_getproc(plugin_handle, "PlayAudio");
        ...
        if (getVersion == NULL || aiDacrateChanged == NULL || aiLenChanged == NULL || initiateAudio == NULL || playAudio == NULL || processAList == NULL ||
            romOpen_audio == NULL || romClosed_audio == NULL || setSpeedFactor == NULL)
        {
...
static m64p_error plugin_connect_gfx(m64p_dynlib_handle plugin_handle)
{
    ...
    else
    {
        ...
        if (getVersion == NULL || changeWindow == NULL || initiateGFX == NULL || moveScreen == NULL || processDList == NULL ||
            ...
            viStatusChanged == NULL || viWidthChanged == NULL || setRenderingCallback == NULL /*||
