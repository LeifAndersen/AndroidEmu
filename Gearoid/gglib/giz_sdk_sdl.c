...
unsigned int gp_getButton(unsigned char enable_diagnals)
{
	...
	unsigned int pad=0;
	int button=0;
	struct _SDL_Joystick *joystick=sdlJoystick;
	
	//Wait until a SDL event is raised
	...
	{
		if (joystick->buttons[button] == SDL_PRESSED) pad|=(1<<button);
	}
	
	if (joystick->hats[0]&SDL_HAT_UP) pad|=(1<<INP_BUTTON_UP);
	if (joystick->hats[0]&SDL_HAT_DOWN) pad|=(1<<INP_BUTTON_DOWN);
	if (joystick->hats[0]&SDL_HAT_LEFT) pad|=(1<<INP_BUTTON_LEFT);
	if (joystick->hats[0]&SDL_HAT_RIGHT) pad|=(1<<INP_BUTTON_RIGHT);
...
{
	Uint32 flags=SDL_FULLSCREEN|SDL_HWSURFACE|SDL_HWPALETTE|SDL_DOUBLEBUF;
	bppMode=bpp;
	sdlScreen=SDL_SetVideoMode(320,240,bpp,flags);
	
	framebuffer16[0]=sdlScreen->pixels;
	framebuffer16[1]=framebuffer16[0];
	framebuffer16[2]=framebuffer16[0];
	framebuffer16[3]=framebuffer16[0];
	framebuffer8[0]=framebuffer16[0];
	framebuffer8[1]=framebuffer16[0];
	framebuffer8[2]=framebuffer16[0];
	framebuffer8[3]=framebuffer16[0];
...
{ 
	SDL_SetPalette(sdlScreen, SDL_PHYSPAL, (SDL_Color*)&gp2x_palette, 0,  256);
} 
