...
draw_tile_loop: 
      ...
      ldrh r9,[r10]
	  ;@ r9 = 0x7FF = tile index
	  ;@ r9 = 0x1000 = vertical flip
	  ;@ r9 = 0x0800 = horizontal flip
	  ;@ r9 = 0x0600 = pal bank 0 - 3
	  
	  ;@ Check if tile is cached
	  
	  ;@ if not cache then get tile cached

	  
      add r3,r3,#1<<16 ;@ increment tile index
      bic r3,r3,r3,lsl#16 ;@ wrap tile index to max tile count
      
      subs r8,r8,#1       ;@ decrement tile rendered count
      bne draw_tile_loop  ;@ if not zero start again
      ldmfd sp!,{pc}  ;@ exit
