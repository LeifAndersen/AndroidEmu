...
static void debug(char *text, int pause)
{
	...
	if(bppmode==8)
	{
		gp_clearFramebuffer8(framebuffer8[Flip],0);
		gp_drawString(0,0,strlen(text),text,0x51,framebuffer16[Flip]);
	}
	else
	{
		gp_clearFramebuffer8(framebuffer16[Flip],0);
		gp_drawString(0,0,strlen(text),text,(unsigned short)RGB(31,31,31),framebuffer16[Flip]);
	}
	MenuFlip();
	if(pause)	MenuPause();
...
void gp_initGraphics(unsigned short bpp, int flip)
{
	...
	*/
	if (!InitFramebuffer)
	{
		...
		// Try the magic Squidge hack
		debug("Patching MMU, please wait....", 0);
		//hackpgtable(); // notaz: we will use something more stable
		mmuhack();
	}
