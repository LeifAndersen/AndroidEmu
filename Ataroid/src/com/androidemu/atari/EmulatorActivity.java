...
import com.androidemu.atari.input.*;
...
public class EmulatorActivity extends Activity implements
	...
	private static final String LOG_TAG = "Ataroid";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final String[] prefKeys = {
			...
			"soundEnabled",
			"soundVolume",
			"leftDifficultySwitch",
			"rightDifficultySwitch",
			"colorSwitch",
			"enableTrackball",
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		...
		} else if ("leftDifficultySwitch".equals(key) ||
				"rightDifficultySwitch".equals(key)) {
			emulator.setOption(key, prefs.getString(key, "B"));

		} else if ("colorSwitch".equals(key)) {
			emulator.setOption(key, prefs.getString(key, "color"));
	...
	private void loadKeyBindings(SharedPreferences prefs) {
		...
		keyboard.mapKey(Emulator.GAMEPAD_FRY,
				prefs.getInt("gamepad_fry", 0));
	}
		...
		try {
			try {
				out = new BufferedOutputStream(
						new FileOutputStream(fileName + ".png"));
