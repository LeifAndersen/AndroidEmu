...
//
// $Id: EventHandler.cxx 2108 2010-08-21 23:08:44Z stephena $
//============================================================================
...
#define GAMEPAD_SELECT	0x0010
#define GAMEPAD_START	0x0020
#define GAMEPAD_FIRE	0x0040
#define GAMEPAD_FRY		0x0080
#define GAMEPAD_MASK	0x00ff

#define CONSOLE_COLOR			0x0100
#define CONSOLE_BW				0x0200
#define CONSOLE_LEFT_DIFF_A		0x1000
#define CONSOLE_LEFT_DIFF_B		0x2000
#define CONSOLE_RIGHT_DIFF_A	0x4000
#define CONSOLE_RIGHT_DIFF_B	0x8000
#define CONSOLE_MASK			0xff00

#define SET_KEY_EVENT(mask, e) \
	if (events & mask) { \
	    myEvent->set(e, (keys & mask) != 0); \
	}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void EventHandler::poll(uInt32 keys)
{
  // Synthesize events for platform-specific hardware
  myOSystem->pollEvent();

  int events = (keys ^ myLastKeyStates);
  myLastKeyStates = keys;

  if (events & GAMEPAD_MASK) {
    SET_KEY_EVENT(GAMEPAD_UP, Event::JoystickZeroUp);
    SET_KEY_EVENT(GAMEPAD_DOWN, Event::JoystickZeroDown);
    SET_KEY_EVENT(GAMEPAD_LEFT, Event::JoystickZeroLeft);
    SET_KEY_EVENT(GAMEPAD_RIGHT, Event::JoystickZeroRight);
    SET_KEY_EVENT(GAMEPAD_FIRE, Event::JoystickZeroFire1);
    SET_KEY_EVENT(GAMEPAD_SELECT, Event::ConsoleSelect);
    SET_KEY_EVENT(GAMEPAD_START, Event::ConsoleReset);
  }
  if (events & CONSOLE_MASK) {
    SET_KEY_EVENT(CONSOLE_COLOR, Event::ConsoleColor);
    SET_KEY_EVENT(CONSOLE_BW, Event::ConsoleBlackWhite);
    SET_KEY_EVENT(CONSOLE_LEFT_DIFF_A, Event::ConsoleLeftDifficultyA);
    SET_KEY_EVENT(CONSOLE_LEFT_DIFF_B, Event::ConsoleLeftDifficultyB);
    SET_KEY_EVENT(CONSOLE_RIGHT_DIFF_A, Event::ConsoleRightDifficultyA);
    SET_KEY_EVENT(CONSOLE_RIGHT_DIFF_B, Event::ConsoleRightDifficultyB);
  }
  if (events >> 16) {
	SET_KEY_EVENT(GAMEPAD_UP << 16, Event::JoystickOneUp);
	SET_KEY_EVENT(GAMEPAD_DOWN << 16, Event::JoystickOneDown);
	SET_KEY_EVENT(GAMEPAD_LEFT << 16, Event::JoystickOneLeft);
	SET_KEY_EVENT(GAMEPAD_RIGHT << 16, Event::JoystickOneRight);
	SET_KEY_EVENT(GAMEPAD_FIRE << 16, Event::JoystickOneFire1);
	SET_KEY_EVENT(GAMEPAD_SELECT, Event::ConsoleSelect);
	SET_KEY_EVENT(GAMEPAD_START, Event::ConsoleReset);
  }
  if (events & GAMEPAD_FRY)
     myFryingFlag = ((keys & GAMEPAD_FRY) != 0);
