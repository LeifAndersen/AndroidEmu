...
public class KeyProfilesActivity extends ListActivity
{
	...
	public static Map<String, Integer> loadProfile(Context context, String name)
	{
		...
		try
		{
			...
			try
			{
				in = new BufferedInputStream(new FileInputStream(getProfileFile(context,
						name)));
				...
				while (event != XmlPullParser.END_DOCUMENT)
				{
							...
							&& parser.getName().equals("key"))
					{
						...
						try
						{
							mappings.put(keyName, new Integer(parser.nextText()));
						}
			...
			Map<String, Integer> mappings)
	{
		try
		{
			...
			try
			{
				out = new BufferedOutputStream(new FileOutputStream(getProfileFile(
						context, name)));
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		switch (item.getItemId())
		{
			...
			case MENU_ITEM_DELETE:
				...
				if (getProfileFile(this, name).delete()) refresh();
				return true;
		}
	...
	protected void onPrepareDialog(int id, Dialog dialog)
	{
		switch (id)
		{
			case DIALOG_EDIT_PROFILE:
				TextView nameView = (TextView) dialog.findViewById(R.id.profile_name);
	...
	private boolean createOrRenameProfile(String oldName, String newName)
	{
		...
		try
		{
			if (oldName == null) return newFile.createNewFile();
			...
			case DIALOG_EDIT_PROFILE:
				DialogInterface.OnClickListener l = new DialogInterface.OnClickListener()
				{
					public void onClick(DialogInterface dialog, int which)
					{
						...
						String newName = ((TextView) d.findViewById(R.id.profile_name))
						...
						else
						{
							Toast.makeText(KeyProfilesActivity.this,
									R.string.new_profile_error, Toast.LENGTH_SHORT)
									.show();
						}
					}
				};
				return new AlertDialog.Builder(this).setTitle(R.string.new_profile_title)
						.setView(getLayoutInflater().inflate(R.layout.new_profile, null))
