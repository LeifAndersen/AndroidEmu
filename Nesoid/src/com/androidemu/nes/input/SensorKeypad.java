...
public class SensorKeypad implements SensorEventListener, Keycodes
{
	...
	private static final float THRESHOLD_VALUES[] = { 30.0f, 20.0f, 15.0f, 10.0f, 8.0f,
	...
	{
		if (gameKeyListener == l) return;

		SensorManager sensorManager = (SensorManager) context
				.getSystemService(Context.SENSOR_SERVICE);

		if (gameKeyListener != null) sensorManager.unregisterListener(this);
	...
	public void onSensorChanged(SensorEvent event)
	{
		...
		else if (leftRight > threshold) states |= Keycodes.GAMEPAD_RIGHT;
		/*
		 * if (upDown < -THRESHOLD) states |= Emulator.GAMEPAD_UP; else if
		 * (upDown > THRESHOLD) states |= Emulator.GAMEPAD_DOWN;
		 */
		if (states != keyStates)
		{
			keyStates = states;
			if (gameKeyListener != null) gameKeyListener.onGameKeyChanged();
		}
	}
}
